    ...
    <section className="space-y-4 apple-style-body">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <article className="card glassmorphism-enhanced p-4 lg:col-span-2">
          <h4 className="font-medium text-readable-title apple-style-heading">即将到期提醒</h4>
          <ul className="mt-3 space-y-3">
            {records.map((item) => (
              <li key={item.pet} className="rounded-md border border-brand-border p-3 text-sm glassmorphism-enhanced">
                <p className="font-medium text-readable-title apple-style-heading">{item.pet} · {item.vaccine}</p>
                <p className="text-readable-muted apple-style-subheading">主人：{item.owner} · 到期：{item.due}</p>
              </li>
            ))}
          </ul>
        </article>
        ...
        <article className="card glassmorphism-enhanced p-4">
          <h4 className="font-medium text-readable-title apple-style-heading">快捷操作</h4>
          <div className="mt-3 grid gap-2">
            {['新建问诊', '上传体检报告', '体重记录', '进入社区'].map((action) => (
              <button key={action} className="focus-ring rounded-md border border-brand-border px-3 py-2 text-left text-sm hover:bg-white/80 backdrop-blur-sm glassmorphism-enhanced text-readable-dark apple-style-body">
                {action}
              </button>
            ))}
          </div>
