...
export function UserPortalPage() {
  return (
    <section className="space-y-4">
      <header>
        <h3 className="section-title">用户端原型</h3>
        <p className="section-desc">聚焦宠物档案、提醒中心、问诊入口与社区快捷入口。</p>
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <article className="card p-4 lg:col-span-2">
          ...
          <ul className="mt-3 space-y-3">
            {records.map((item) => (
              <li key={item.pet} className="rounded-md border border-brand-border p-3 text-sm">
                <p className="font-medium">{item.pet} · {item.vaccine}</p>
                <p className="text-brand-muted">主人：{item.owner} · 到期：{item.due}</p>
              </li>
            ))}
          </ul>
        </article>
